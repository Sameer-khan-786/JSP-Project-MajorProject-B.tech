...
public class MessageDAO1 {
	...
	public ArrayList<Message1> getAllMessage(int id) throws SQLException {
		...
		PreparedStatement st = conn.prepareStatement("SELECT * FROM message1 WHERE from_user = ? AND chat_time IN (SELECT MAX(chat_time) FROM message GROUP by to_user) ORDER BY chat_time DESC;");
		st.setInt(1, id);
	...
	public void deleteAllMessage(int fromUserID, int toUserID) throws SQLException {
		Connection conn = DBConnection.getInstance().getConnection();
		PreparedStatement st = conn.prepareStatement("DELETE FROM message1 WHERE (from_user = ? AND to_user = ?) OR (to_user = ? AND from_user = ?);");
	...
	public ArrayList<Message1> getMessage(int fromUserID, int toUserID) throws SQLException {
		Connection conn = DBConnection.getInstance().getConnection();
		PreparedStatement st = conn.prepareStatement("SELECT * FROM message1 WHERE (from_user = ? OR to_user = ?) AND (from_user = ? OR to_user = ?) ORDER BY chat_time;");
		st.setInt(1, fromUserID);
	...
	public void insertMessage(int fromUserID, int toUserID, String message) throws SQLException{
		Connection conn = DBConnection.getInstance().getConnection();
		PreparedStatement st = conn.prepareStatement("INSERT INTO message1(from_user, to_user, message) VALUES (?,?,?);");
		st.setInt(1, fromUserID);
		st.setInt(2, toUserID);
	...
	public void deleteMessage(int chatID) throws SQLException{
		Connection conn = DBConnection.getInstance().getConnection();
		PreparedStatement st = conn.prepareStatement("DELETE FROM message1 WHERE chat_id = ?;");
		st.setInt(1, chatID);
		st.executeUpdate();
	}
	...
	public int messageCount(int from_user, int to_user) throws SQLException {
		int count = 0;
		Connection conn = DBConnection.getInstance().getConnection();
		PreparedStatement st = conn.prepareStatement("SELECT COUNT(*) msg_count FROM message1 WHERE (from_user = ? OR to_user = ?) AND (from_user = ? OR to_user = ?);");
